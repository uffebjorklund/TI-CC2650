...
Since the library we use the communicate with the sensortag from the Raspberrry Pi is based on NodeJS, we will use NodeJS for real-time communication as well.
...
---
#The solution
The mission with this article was to show how to read/write in full-duplex to CC2650 from anywhere in the world. To be able to do so we need three parts.
...
##Real-Time Server
Since XSockets.NET has state, can connect anything and allows you to talk cross-protocol etc it will be very easy to build the server-side communication.
...
![enter image description here](http://xsockets.net/$2/file/sensor-controller-1.png)
...
![enter image description here](http://xsockets.net/$2/file/monitor-controller-1.png)
...
The "black" window is just the output from the Raspberry Pi connected via Putty.
![enter image description here](http://xsockets.net/$2/file/ir-temp-live.png)
